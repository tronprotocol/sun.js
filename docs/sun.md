...
To use the Sun library in your App, you need to instantiate Sun. 
...
Supposing you are using a server which provides everything, like TronGrid, you can instantiate Sun as:
...
For example, you can create a sun instance connected to out sun network test-net like:
...
```javascript
// Format
sun.depostiTrx(callValue, feeLimit, options);

// example
sun.depositTrx(100000000, 1000000);
```
...
```javascript
// format
sun.depositTrc10(tokenId, tokenValue, feeLimit, options);

// example
sun.depositTrc10(100059, 10000000, 100000);
```
...
Note: You have to mapping TRC20 contract to side chain with the mappingTrc20 function provided by Sun. Then you also have to use  the approveTrc20 function. Only done with the two steps before, you can depositTrc20 from main chain to side chain.

```javascript
// format
sun.depositTrc10(num, feeLimit, contractAddress, options);

// example
sun.depositTrc10(1000000, 1000000, 'TD9Jrm546pGkzRu7K5nitMxk8nn75wXNkQ');
```
...
```javascript
// format
sun.depositTrc721(id, feeLimit, contractAddress, options);

// example
sun.depositTrc10(1000000, 1000000, 'TCLRqK6aP2xsCZWhE2smkYzdRHf9uvyz5P');
```
...
```javascript
// format
sun.approveTrc20(num, feeLimit, contractAddress, options);

// example
sun.approveTrc20(10000, 10000000, 'TGKuXDnvdHv9RNE6BPXUtNLK2FrVMBDAuA');
```
...
```javascript
// format
sun.approveTrc721(id, feeLimit, contractAddress, options);

// example
sun.approveTrc721(100, 10000000, 'TUxDmFbEceGgjWCb6rLVcrFgnsWwofPdPq');
```
...
```javascript
// Format
sun.withdrawTrx(callValue, feeLimit, options);

// example
sun.withdrawTrx(100000000, 1000000);
```
...
```javascript
// format
sun.withdrawTrc10(tokenId, tokenValue, feeLimit, options);

// example
sun.withdrawTrc10(100059, 10000000, 100000);
```
...
```javascript
// format
sun.withdrawTrc20(num, feeLimit, contractAddress, options);

// example
sun.withdrawTrc20(10000, 10000000, 'TWzXQmDoASGodMss7uPD6vUgLHnkQFX7ok');
```
...
```javascript
// format
sun.withdrawTrc721(id, feeLimit, contractAddress, options);

// example
sun.withdrawTrc721(101, 10000000, 'TA2xrVESq2UcEtDtgPzxNJEiLgxmMVdtFR');
```
