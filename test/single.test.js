...
describe('Sun Instance', function() {
    ...
    describe('#fullnodeVersion()', function () {
        it('should verify that the version of the fullNode is available', function () {
            ...
            assert.equal(typeof sun.mainchain.fullnodeVersion, 'string');
    ...
    describe('#setDefaultBlock()', function () {
        it('should accept a positive integer', function () {
            ...
            sun.mainchain.setDefaultBlock(1);

            assert.equal(sun.mainchain.defaultBlock, 1);
        });

        it('should correct a negative integer', function () {
            const sun = sunBuilder.createInstance();

            sun.mainchain.setDefaultBlock(-2);

            assert.equal(sun.mainchain.defaultBlock, 2);
        });

        it('should accept 0', function () {
            const sun = sunBuilder.createInstance();

            sun.mainchain.setDefaultBlock(0);

            assert.equal(sun.mainchain.defaultBlock, 0);
        });

        it('should be able to clear', function () {
            const sun = sunBuilder.createInstance();

            sun.mainchain.setDefaultBlock();

            assert.equal(sun.mainchain.defaultBlock, false);
        });

        it('should accept "earliest"', function () {
            const sun = sunBuilder.createInstance();

            sun.mainchain.setDefaultBlock('earliest');

            assert.equal(sun.mainchain.defaultBlock, 'earliest');
        });

        it('should accept "latest"', function () {
            const sun = sunBuilder.createInstance();

            sun.mainchain.setDefaultBlock('latest');

            assert.equal(sun.mainchain.defaultBlock, 'latest');
        });

        it('should reject a decimal', function () {
            const sun = sunBuilder.createInstance();

            assert.throws(() => sun.mainchain.setDefaultBlock(10.2), 'Invalid block ID provided');
        });

        it('should reject a string', function () {
            const sun = sunBuilder.createInstance();

            assert.throws(() => sun.mainchain.setDefaultBlock('test'), 'Invalid block ID provided');
        });
    });
    ...
    describe('#isValidProvider()', function () {
        it('should accept a valid provider', function () {
            ...
            const provider = new sun.mainchain.providers.HttpProvider(MAIN_FULL_NODE_API);
    ...
    describe('#setFullNode()', function () {
        ...
        it('should accept a valid URL string', function () {
            ...
            const provider = MAIN_FULL_NODE_API;
            sun.mainchain.setFullNode(provider);
    ...
    describe('#setSolidityNode()', function () {
        it('should accept a HttpProvider instance', function () {
            ...
            const provider = new sun.mainchain.providers.HttpProvider(MAIN_SOLIDITY_NODE_API);
        ...
        it('should accept a valid URL string', function () {
            ...
            const provider = MAIN_SOLIDITY_NODE_API;
            sun.mainchain.setSolidityNode(provider);
        ...
        it('should reject a non-string', function () {
            assert.throws(() => {
                ...
                sunBuilder.createInstance().mainchain.setSolidityNode(true)
        ...
        it('should reject an invalid URL string', function () {
            assert.throws(() => {
                ...
                sunBuilder.createInstance().mainchain.setSolidityNode('_localhost')
            }, 'Invalid URL provided to HttpProvider');
        });
    });
    ...
    describe('#setEventServer()', function () {
        it('should accept a valid URL string', function () {
            ...
            const eventServer = MAIN_EVENT_API;
    ...
    describe('#currentProviders()', function () {
        it('should return the current providers', function () {
            ...
            // 以下参数添加了前缀 MAIN_
            assert.equal(providers.fullNode.host, MAIN_FULL_NODE_API);
            assert.equal(providers.solidityNode.host, MAIN_SOLIDITY_NODE_API);
            assert.equal(providers.eventServer.host, MAIN_EVENT_API);
        });
    });
    ...
    describe('#sha3()', function () {
        it('should match web3 sha function', function () {
            ...
            const sun = sunBuilder.createInstance();    // 同上
    ...
    describe("#createAccount", function () {
        it("should create a new account", async function () {
            ...
            const newAccount = await sun.createAccount();   // 此处已改为小写
